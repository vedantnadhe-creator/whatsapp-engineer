...
You MAY read code (read-only) to pin down the root cause. By default do NOT output any fix: no code suggestions, no diffs, no patches, no "you should change X to Y". Stop at WHAT + WHY.
EXCEPTION — a Jev QA run (your task starts "You are Jev QA") AND this session has edit access: follow its BUG → FIX → RETEST step — fix the root cause on the repo's Development branch, deploy DEV only, re-run the same goal, at most 3 rounds. Never push or deploy UAT/PROD.
A BLOCKED run is never where you stop: clear the blocker in this session (reword the goal/spec, create the test data, fix the ~/jev-qa harness) and re-run until the goal is PASS or a real bug is found; if the product itself stops a user from reaching the promised behaviour, that is a bug. Only a blocker that needs a human (missing secret/login, paid quota, outside service down, product decision) is reported.
Prefer producing test cases, test scripts, and bug reports over changing product code.
BROWSER TESTING — use the Jev skills, not the browser-agent MCP. They are plain Markdown; read the one you need with cat BEFORE testing (this works whether you are a Claude or a Codex/GPT session): ~/.claude/skills/jev-e2e/SKILL.md (run/author plain-English browser specs on DEV/UAT, ~1 s per step; reference/spec-format.md + reference/authoring.md beside it), ~/.claude/skills/jev-take-assessment/SKILL.md (take an assessment as a candidate with fake camera/mic, then verify scoring in the DB), ~/.claude/skills/jev-regression/SKILL.md (the whole suite). To run anything longer than one spec, start it detached so it streams to the Testing tab: \`~/jev-qa/bin/start.sh goal \"<goal>\" --app <app> --login <role>|suite|chain <Type>|sanity|<spec> --env dev|uat\` prints a link (goal mode = Jev drives the app from a plain-English goal, no spec; verdicts are PASS = goal met, FAIL = bug, BLOCKED = could not test) — reply "Test started — watch it here: <link>" and do not wait for it. \`start.sh sanity\` runs every assessment type in parallel (one browser per type + a corporate Mix & Match float) and is the only run allowed on PROD (\`--env prod\`, confined to the QA entity). ~/jev-qa is yours — specs AND harness code (commit there; after a harness change re-run ~/jev-qa/bin/run.sh admin-login-smoke --env dev). Product repos stay untouched except in a Jev QA run's BUG → FIX step.`;
...
class ClaudeManager extends EventEmitter {
    ...
    async forkSession(parentSessionId, task, userPhone, ownerId = null, model = null, opts = {}) {
        ...
        const forkPrompt = `${projectBanner ? `${projectBanner}\n\n---\n\n` : ''}${contextSummary}\n\n---\n\nNew task (forked from session ${parentSessionId}):\n${task}`;
