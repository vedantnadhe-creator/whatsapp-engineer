...
export function jevTask({ env = 'dev', device = 'pc', browser = 'chromium', notes = '', subject, deployed = true }) {
    ...
    return [
        ...
        `3. VERDICT per goal = run.json "result": PASS = goal met · FAIL = bug (the product did the wrong thing — error screen, failing API, wrong screen, false assertion) · BLOCKED = the test was stopped by something (timeout, element not found, missing data, harness) — not a verdict on the product, and never where you stop (step 5). "reason" says which step and why. A goal with a failing step is not met, however many other steps passed. Every goal must end PASS or BUG.`,
        ...
        `5. BLOCKED IS NOT AN ENDING — clear the blocker yourself in this session and re-run, until the goal is PASS or a real bug is found. Read the run's "reason" and its screenshots, then: (a) TEST-SIDE blocker → fix it and re-run: vague goal → rewrite it more specifically; element not found → reword the spec or the goal to what is really on screen; wrong app/login/start page → correct the flags; flow longer than the budget → --max 60 or split the goal; missing test data → create it through the app or a float spec (QA names, QA candidate only); harness crash/timeout/bug → fix ~/jev-qa code (it is yours: commit there, then run ~/jev-qa/bin/run.sh admin-login-smoke --env dev to prove nothing else broke). (b) the product itself stops a user from reaching what the change promised (feature missing or unreachable, page never loads, endless spinner) → that is a BUG: step 4. (c) only a blocker that needs a human — a secret or login that is not in ~/jev-qa/.env, a paid third-party quota, a service outside DEV/UAT that is down, a product decision — is NOTIFIED, with exactly what is needed; keep testing the other goals meanwhile. Up to 5 unblock attempts per goal; each attempt must change something, never re-run the same thing unchanged.`,
        `6. REPORT, in this order: a table Goal | Verdict | Run link; then "Bugs fixed" (root cause, file/commit, DEV retest link); "Bugs open" (WHAT + WHY); "Blockers cleared" (what stopped the test and how you unblocked it); "Needs a human" (only type-c blockers, with what is needed). The work is QA-passed only when every goal is PASS.`,
    ].filter(Boolean).join('\n');
}
